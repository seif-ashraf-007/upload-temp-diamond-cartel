...
const QuickFilters = ({
  ...
  const debouncedCaratChange = useCallback(
    debounce((min, max) => {
      ...
      if (max === "") {
        ...
        finalMax = isNaN(numMax) ? null : Math.min(10, Math.max(minValue, numMax));
      }
  ...
  const handleCaratSliderChange = (values) => {
    ...
    console.log('Slider changed to:', sliderMin, sliderMax, '-> Carats:', min, max);
          ...
          <div>
            ...
            <div className="px-1">
              <Slider
                range
                min={0.01}
                max={5}  /* We use a smaller max for the slider but convert to full range */
                step={0.01}
                defaultValue={[0.01, 5]}
                value={[
